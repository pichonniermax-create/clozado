...
import { db } from "@/db";
import { contacts, deals, partners } from "@/db/schema";
 ...
 * — contacts, affaires, partenaires — limité à quelques résultats chacun,
 ...
 */
export type SearchHit = { kind: "contact" | "deal" | "partner"; id: string; title: string; subtitle: string | null; href: string };
...
export async function searchEverything(user: OrgScopeUser, query: string): Promise<SearchHit[]> {
  ...
  const [contactRows, dealRows, partnerRows] = await Promise.all([
    db
